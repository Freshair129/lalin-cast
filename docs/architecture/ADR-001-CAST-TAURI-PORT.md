...
| Portable mode (not a VacuumTube feature; new in wave 11, the one remaining item that needed no founder/purchase decision) | Rust `src-tauri/src/portable.rs` — `detect(exe_dir: &Path) -> Mode` pure fn deciding `Installed` vs. `Portable { data_root }` from a `lalin-cast.portable` marker read only next to `std::env::current_exe()`, decided once into a `OnceLock`; a single helper routes every `app.store("media-settings.json")` call site, the log/`lifecycle.json` directories and every `WebviewWindowBuilder`'s WebView2 `data_directory` through the same `<data_root>` when portable, with installed-mode behavior byte-for-byte unchanged; `startWithWindows`/`deepLinkScheme` reconcile-on-start is skipped and `apply_setting`/`cast_update_install` return a fixed `"not available in portable mode"` error before touching the registry or downloading anything; `SettingsSnapshot`/update-window payload/diagnostics each gain a `portable: bool` field (diagnostics omits the data-root path) | wave 11 local slice — see `docs/plans/W11_PORTABLE_PLAN.md` | `portable.rs` unit tests (no-marker/marker-writable/marker-unwritable-falls-back-to-installed) on temp dirs plus source-scan tests (`store("media-settings.json")` string literal appears nowhere outside `portable.rs`, every `WebviewWindowBuilder::new(` goes through the data-directory helper); a CI portable smoke job runs the real debug exe with the marker present and fails if `lifecycle.json` doesn't land beside the exe or if the installed-mode `%LOCALAPPDATA%\ai.lalin.cast\lifecycle.json` is touched at all; human gate H29 (run the dry-run portable zip from a USB drive on a clean machine) |
| Release assets (not a VacuumTube feature; new in wave 12, closes the gap where `release.yml` had never run and none of the dry run's build-check logic lived in it) | Five shared PowerShell scripts under `scripts/release/` (`Test-ReleaseVersion.ps1` — tag-vs-app-version and CHANGELOG-section guard, with a dry-run-only `-SkipChangelog` switch; `Test-InstallerScheme.ps1` — the fail-closed `installer.nsi`/`Classes\lalin-cast` check moved out of the dry run unchanged; `New-PortableZip.ps1` — builds and self-verifies the three-file portable zip for either architecture; `Write-Checksums.ps1` — sorted, LF-terminated `sha256sum`-format checksum files; `Publish-ReleaseAssets.ps1` — `gh release upload`/`-WhatIf` asset attachment) plus a fixture-driven `selftest.ps1` covering every pass/fail case, run as a blocking `ci.yml` job on every pull request; `release-dryrun.yml` now calls the same scripts for both architectures (with `-SkipChangelog`, since no tag exists at dry-run time); `release.yml` gains the version/CHANGELOG guard as its first step, before any Rust check or build, and attaches the portable zip plus a checksum file per architecture to the real draft release, without removing or loosening a single existing step | wave 12 local slice — see `docs/plans/W12_RELEASE_ASSETS_PLAN.md` | `[System.Management.Automation.Language.Parser]::ParseFile` on every script; `selftest.ps1` exit 0 locally and in CI; `Test-ReleaseVersion.ps1 -Tag v0.1.0 -SkipChangelog` passes and `-Tag v9.9.9` fails against the real repo; YAML-parses and SHA-pin checks on all three touched workflow files; a diff confirming every pre-wave-12 step and setting in `release.yml` is still present and unweakened; `src-tauri/**`/`fallback/**`/`Cargo.lock`/`THIRD_PARTY_NOTICES.md` unchanged; human gate H30 (download and verify every asset from the very first real draft release before publishing it) |
...
| 0.18.0b | 2026-09-21 | beta | Wave 11: added the feature-matrix row for portable mode — a `lalin-cast.portable` marker read only next to the exe, deciding once between installed (byte-for-byte unchanged) and a `<data_root>` that carries the settings store, log, `lifecycle.json` and every window's WebView2 profile; `startWithWindows`/`deepLinkScheme` refused in Rust before touching the registry, in-place update install refused before downloading, and a `portable: bool` added to the settings/update/diagnostics payloads with no data-root path in diagnostics; no new crate/feature/`unsafe` (see `docs/plans/W11_PORTABLE_PLAN.md`) | uncommitted | LALIN |
| 0.19.0b | 2026-09-21 | beta | Wave 12: added the feature-matrix row for release assets — five shared, self-tested `scripts/release/` PowerShell scripts (tag/version/CHANGELOG guard, fail-closed installer-scheme check, portable-zip build, SHA-256 checksums, asset upload with `-WhatIf`) that both the dry run and `release.yml` now call identically, `release.yml` gaining the version/CHANGELOG guard before any build step and portable-zip/checksum assets per architecture without loosening any existing step; no new crate/feature/`unsafe`, no Rust or UI change (see `docs/plans/W12_RELEASE_ASSETS_PLAN.md`) | uncommitted | LALIN |
