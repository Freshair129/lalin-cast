...
- [`plans/W12_RELEASE_ASSETS_PLAN.md`](plans/W12_RELEASE_ASSETS_PLAN.md) — Wave 12 release assets DAG (moving the dry run's inline packaging checks into shared, self-tested `scripts/release/` PowerShell scripts that both `release-dryrun.yml` and `release.yml` call identically, a tag-vs-app-version/CHANGELOG-section guard that fails `release.yml` before any build starts, and portable-zip plus SHA-256-checksum assets attached to the draft release for both architectures without loosening any existing `release.yml` step), constants/contracts, file ownership matrix and parallel-stream/verify-gate plan.
