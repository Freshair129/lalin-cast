...
- [`ADR-001-CAST-TAURI-PORT.md`](architecture/ADR-001-CAST-TAURI-PORT.md) — Rust + Tauri v2 shell, Leanback surface, DIAL boundary, feature gates and, added in wave 9, the local rotating log file's single-sanitiser guarantee, extended in wave 10 to also mask the Windows home folder and account name and to record the unsigned release-dry-run workflow.
